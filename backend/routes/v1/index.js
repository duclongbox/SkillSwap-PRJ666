  ...
  try {
    ...
    if (!title || !category || !description || !exchangeSkills) {
      return res.status(400).json({
        message:
          "Title, category, description, and exchange skills are required",
      });
    }
    ...
    const skill = new Skill({
      ...
      exchangeSkills: Array.isArray(exchangeSkills)
        ? exchangeSkills
        : [exchangeSkills],
...
router.put("/api/skills/:id", isAuthenticated, async (req, res) => {
  try {
    ...
    if (
      skill.owner_id.toString() !== req.user._id.toString() &&
      req.user.role !== "admin"
    ) {
      return res
        .status(403)
        .json({ message: "Unauthorized to update this skill" });
    }
